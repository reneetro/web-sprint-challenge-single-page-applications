...
useRouteMatch is used for creating paths that are relative to one another based on a url in a component.
1. How would you explain form validation to someone who has never programmed before?
Validation makes it so there is very little to no room for a user to give the form an invalid input. It makes sure that all data that gets sent is in the correct format and provides the correct information.
1. In 1-2 sentences, define end to end testing.
End to end testing is used to test the flow of an application from start to finish and makes sure it is behaving as it should. Testing should mimic how a user will be interacting with the software to identify any potential issues.
